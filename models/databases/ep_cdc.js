...
listDimensionTables = async () => {
    var query = `SHOW TABLES`
    var result = []
    await new Promise((resolve, reject) => {
        connection.query(query, (err,rows, fields) => {
            if(!err) resolve(JSON.parse(JSON.stringify(rows)))
            else reject(err)
        })
    }).then(response => {
        var dimensions = []
        response.forEach(value => {
            var dimension = value.Tables_in_ep_cdc
            if(dimension.includes('bi_dim')){
                dimensions.push(value.Tables_in_ep_cdc)
            }
        })
        result = dimensions
    })
